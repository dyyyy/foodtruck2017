						...
						<div class="pagination" >
							<ul>
								<%
									...
									if (startPage > 1) {
								%>
								<li><a href="search?pageNo=1&search=<%=request.getAttribute("search")%>">처음</a></li>
								<%
									}
									if (currentPage > 1) {
								%>
								<li><a href="search?pageNo=<%=currentPage - 1%>&search=<%=request.getAttribute("search")%>"><i class="fa fa-angle-left"></i></a></li>
								<%
									}
								...
								%>
								<li><a href="search?pageNo=<%=iCount%>&search=<%=request.getAttribute("search")%>"><%=iCount%></a><li>
								<%
									}
									}
									if (currentPage < totalPage) {
								%>
								<li><a href="search?pageNo=<%=currentPage + 1%>&search=<%=request.getAttribute("search")%>"><i class="fa fa-angle-right"></i></a></li>
								<%
									}
									if (endPage < totalPage) {
								%>
								<li><a href="search?pageNo=<%=totalPage%>&search=<%=request.getAttribute("search")%>">끝</a></li>
								<%
									}
								%>
								</ul>
